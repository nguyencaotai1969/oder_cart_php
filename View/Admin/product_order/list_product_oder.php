        ...
        <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered dataTable dtr-inline" role="grid" aria-describedby="example_info">
         ...
         <tbody>
          ...
          foreach ($product_oder as $sp) {
           ...
           echo '<a href="?controller=admin&action=Change_product_oder&id=' . $sp['id'] . '" Onclick="refresh( ' . $sp['id'] . ')" class="btn btn-primary"><i class="fa fa-cog "></i></button>';
           echo "</td>";
           echo "</tr>";
          }
...
<script type="text/javascript">
 function ConfirmDelete(id) {
  ...
  if (x) {
   $.ajax({
    type: "POST",
    url: "?controller=Admin&action=Delete_product_oder",
    data: {
     id: id
    },
