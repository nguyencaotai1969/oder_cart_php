...
class AdminModel extends Database
{
 ...
 {
  $conn = $this->conn();
  $result = mysqli_query($conn, "SELECT * FROM product where product_id = 1 and id = $id");
  $data = [];
  if ($result && $result->num_rows > 0) {
 ...
 public function change_product_sale($id)
 {
  $conn = $this->conn();
  $result = mysqli_query($conn, "SELECT * FROM product where product_id = 4 and id = $id");
  $data = [];
  if ($result && $result->num_rows > 0) {
